...
int myFSFormat(Disk *d, unsigned int blockSize)
{
	...
	{
		printf("\n!! Error: Block size (%u) must be multiple of sector size (%d). Disk ID: %d\n",
	...
	unsigned int inodesSectors = (numInodes + inodeNumInodesPerSector() - 1) / inodeNumInodesPerSector();
	...
	if (totalClusters < 2)
	{
		...
		printf("   Data sectors available: %lu, Clusters: %u\n", dataSectors, totalClusters);
		return -1;
	}

	printf("\n   Layout calculated:");
	printf("\n   - Inodes: %d (sectors %u to %u)", numInodes, inodesBeginSector, dataBeginSector - 1);
	...
	for (unsigned long i = dataBeginSector; i < totalSectors; i++)
	{
		...
		if ((i - dataBeginSector) % sectorsPerCluster == 0)
		{
			...
			memcpy(emptySector, &header.nextClusterAddress, sizeof(unsigned long int));
		}
...
int myFSxMount(Disk *d, int x)
{
	...
	if (x == 1)
	{
		...
		if (diskReadSector(d, 0, sector) != 0)
			return 0;
		if (sector[0] != 'M' || sector[1] != 'Y' || sector[2] != 'F' || sector[3] != 'S')
			return 0;
